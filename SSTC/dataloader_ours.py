...
class IcaAVELoader(Dataset):
    def __init__(self, args, mode='train', modality='visual', incremental_step=0):
        ...
        if args.dataset == 'AVE':
            self.data_root = ""
            self.visual_pretrained_feature_path = os.path.join(self.data_root,'visual_pretrained_feature.npy')
        ...
        self.audio_pretrained_feature_path =  os.path.join(self.data_root, 'audio_pretrained_feature.npy')
...
class exemplarLoader(Dataset):
    ...
    def _update_exemplars_(self):
        ...
        exemplar_num_per_class = self.args.memory_size
